...
Many stochastic programming models can be written as:
...
Where $X\subset\RR^m$ is closed, $\Xi\subset\RR^s$ is closed, $F_j$ are random lsc functions for all $j\in\{0,...,d\}$ and $P$ is a Borel measure. The set $X$ does not depend on P and defines constraints on $x$. The goal of this part is to highlight a specific metric on distributions that would ensure that, if two distributions $\mathbb{P}, \mathbb{Q}$ are close, then the optimum of both models are close. To make it, we write:
\begin{align*}
...
\end{align*}
So basicall, $v\left(P\right)$ denotes the optimal value, $X^*\left(P\right)$ the argmin set. Let $\mathcal{F}$ a class of measurable functions from $\Xi$ to $\RR$, a pseudometric quantifying distances between distributions is:
...
This pseudometric can be a metric if $\mathcal{F}$ preserves the definition: $d_\mathcal{F}\left(P,Q\right)=0\implies P=Q$. An important family that we will use in this part is the family of locally Lipschitz continuous functions defined as: $\mathcal{F}_p\left(\Xi\right)$ for $p\geq1$: 
$$
\mathcal{F}_p\left(\Xi\right)=\left\{F:\Xi\to \RR: \lvert F\left(\xi\right)-F\left(\tilde{\xi}\right)\rvert \leq c_p\left(\xi,\tilde{\xi}\right)\lVert\xi-\tilde{\xi}\rVert, \forall \xi,\tilde{\xi}\in \Xi^2 \right\},
$$
the parameter $p$ defines $c_p\left(\xi,\tilde{\xi}\right)=\max\left\{1,\lVert \xi\rVert, \lVert \tilde{\xi}\rVert\right\}^{p-1}$. We can now write $p$-th order Fortet-Mourier metrics, our first metrics on distributions:
$$
...
\end{equation*}
which is basically \ref{stochastic} but with another distribution $P \to Q$. In this part we'll try to compare the notions we've presented in the part above, for instance, we want to bound $\lvert v\left(P\right)-v\left(Q\right)\rvert$ in terms of distribution metrics. \cite[Corollary 14]{romisch_stability_2003} gives:
\begin{theorem}\label{stability}
    Let $d=0$ and assume that:
    \begin{enumerate}
        \item $X^*\left(P\right)$ is nonempty and $\mathcal{U}$ is an open, bounded neighbourhood of $X^*\left(P\right)$.
        \item $X$ is convex and $F_0\left(\cdot,\xi\right)$ is convex on $\RR^m$ for each $\xi\in\Xi$.
...
\end{theorem}
This theorem (which is a corollary in the article) highlights two main aspects. Under the assumptions made and $d=0$, if you slightly perturb $P$ (meaning $Q$ is close to $P$ for the $p$-th order Fortet-Mourier metric), the optimal value is controlled, it can't change a lot. Another thing is that the set of optimal $x$ with $Q$ will remain close to the set of optimal $x$ with $P$. This is a result of stability. 

\subsection{Application to linear two-stage models}
...
\begin{equation}\label{lp stoch}
    \min\left\{c^Tx + \int_\Xi <q\left(\xi\right), y\left(\xi\right)> \text{d}P\left(\xi\right)\: :\: Wy\left(\xi\right)=h\left(\xi\right)-T\left(\xi\right)x\:,\: y\left(\xi\right)\:,\: x\in X \right\}.
\end{equation}
Where $x$ is the first stage decision variable, $y$ the second. In order to fit in the definition of  \ref{stochastic}, we define:
$$
F_0\left(x,\xi\right)=\begin{cases} 
  \langle c, x \rangle + \phi(q(\xi), h(\xi) T(\xi)x) & h(\xi) T(\xi)x \in \text{pos}\left(W\right), q(\xi) \in D, \\
  +\infty & \text{otherwise},
\end{cases}
$$
where $\text{pos}(W)=\left\{Wy, y\in\RR_+^m\right\}$, $D=\left\{u\in\RR^m:\left\{z\in\RR^r:W^Tz \leq u\right\}\ne \emptyset\right\}$ and $\phi\left(u,t\right)=\inf\left\{\langle u,y\rangle: Wy=t, y\geq0\right\}$. \ref{lp stoch} can be rewritten as:
\begin{equation}
...
\begin{enumerate}
    \item For each $\left(x,\xi\right)\in X\times \Xi$ it holds that $h(\xi) T(\xi)x) h(\xi) T(\xi)x \in \text{pos}\left(W\right), q(\xi) \in D$.
    \item $P\in\mathcal{P}\left(\Xi\right)$ and has a finite $2$-th moment.
\end{enumerate}

\begin{corollary}
    Let both assumptions be satisfied and let $X^*\left(P\right)$ be nonempty and $\mathcal{U}$ be an open, bounded neighbourhood of $X^*\left(P\right).$ Then there exist constants $L>0$ and $\delta >0$ such that:
    \begin{align*}
        \lvert v\left(P\right)-v\left(Q\right)\rvert \leq L\zeta_2\left(P,Q\right) \\
        \emptyset \ne X^*\left(Q\right)\subset X^*\left(P\right)+\Psi_P\left(L\zeta_2\left(P,Q\right)\right)\mathbb{B}
    \end{align*}
    whenever $Q\in\mathcal{P}\left(\Xi\right)$ and has a finite $2$-th moment and $\zeta_2\left(P,Q\right)<\delta$.
\end{corollary}
\begin{proof}
    This is an application of \Cref{stability} with $p=2$ where both assumptions are sufficient to ensure that conditions (2), (3) of \Cref{stability} are verified. See \cite[Proposition 22]{romisch_stability_2003}.
\end{proof}
If we take a step back and consider a two-stage model with a distribution $P=\sum_{i\in I}\delta_{x_i}p_i$ with $n$ atoms with $n$ large. We may want to find another distribution close to $P$ (in terms of $\zeta_2$) so that $\lvert v\left(P\right)-v\left(Q\right)\rvert$ is controlled but with a reduced number of atoms to reduce run time when solving it on a computer. If you remember the definition of $p$-th order Fortet-Mourier metric:
$$
\zeta_2\left(P,Q\right)=\sup_{F\in\mathcal{F}_2\left(\Xi\right)}\lvert \int_\Xi F\left(\xi\right)\text{d}P\left(\xi\right)-\int_\Xi F\left(\xi\right)\text{d}Q\left(\xi\right)\rvert.
$$
where $\mathcal{F}_2\left(\Xi\right)=\left\{F:\Xi\to \RR: \lvert F\left(\xi\right)-F\left(\tilde{\xi}\right)\rvert \leq c_2\left(\xi,\tilde{\xi}\right)\lVert\xi-\tilde{\xi}\rVert, \forall \xi,\tilde{\xi}\in \Xi^2 \right\}$. It doesn't look very friendly. The idea is to bound $\zeta_2\left(P,Q\right)$ by another metric  the type-2 Wasserstein distance. We write $D\left(P,Q\right)$ probability measures for which marginals are $P$ and $Q$.
\begin{align*}
    \zeta_2\left(P,Q\right)&= \sup_{F\in\mathcal{F}_2\left(\Xi\right)}\lvert \int_\Xi F\left(\xi\right)\text{d}P\left(\xi\right)-\int_\Xi F\left(\xi\right)\text{d}Q\left(\xi\right)\rvert \\
    &\leq \sup_{F\in\mathcal{F}_2\left(\Xi\right)} \int_{\Xi \times \Xi} \lvert F\left(\xi\right)-F\left(\xi'\right)\rvert \eta\left(\text{d}\xi\times \text{d}\xi'\right), \text{ with } \eta\in D\left(P,Q\right) \\
    &\leq \int_{\Xi \times \Xi} \max\left\{1,\lVert \xi\rVert, \lVert \xi'\rVert\right\} \lVert \xi -\xi'\rVert \eta\left(\text{d}\xi\times \text{d}\xi'\right), \text{ because } F\in\mathcal{F}_2\left(\Xi\right) \\
    & \leq \left( \int_{\Xi \times \Xi}\max\left\{1,\lVert \xi\rVert, \lVert \xi'\rVert\right\}^2\eta\left(\text{d}\xi\times \text{d}\xi'\right)\right)^{1/2}\left(\int_{\Xi \times \Xi}\lVert \xi -\xi'\rVert^2\eta\left(\text{d}\xi\times \text{d}\xi'\right)\right)^{1/2} \\
    &\leq K^2d_2\left(P,Q\right), \text{ where }K=\max_{\xi\in\Xi}{\lVert \xi\rVert}
\end{align*}
This new metric $d_2$ is the solution of an optimal transport problem ! Looks better.
...
The "x" axis of \Cref{comparison dup} represents the number of atoms in the reduced set. \Cref{dist for bac} shows that the efficiency in term of Wasserstein distance is similar and comparable for a reduced set obtained via forward or backward algorithm but still Forward algorithm gets a slight lead. The main difference lies in calculation time, the efficient way to compute the forward algorithm shows its advantages in \Cref{time for bac}, always below 0.05s whereas the backward algorithm can take up to one second and takes more time than the exact method. Forward algorithm has to be preferred when you're free to choose.  This data-set of 100 atoms is small compared to what you could see in real problems, the calculation time will grow as $n$ grows, that's why I make this remark. We use Gurobi to solve the MILP formulation and when there is 100 atoms, heuristics are used to find a solution (that's why the computation time is very low compared to what we could expect). 

\subsection{Comparison with a bigger $n$}
We've seen that backward algorithm isn't interesting in terms of run time. We'll compare now the run time of the Forward algorithm and the MILP formulation for $n=500$ because \Cref{time for bac} isn't relevant as Gurobi is very efficient for small size. Let's say we want to reduce a $n$ atoms distribution to a $\frac{n}{4}$ atoms distribution, let's compare run time and efficiency for $n\in\left[100,1000\right]$:
...
\Cref{complexity} tells us that forward Dupačová has a complexity of $\mathcal{O}\left(mn^2\right)$ what's meant to be far less than a simplex, Gurobi may use heuristics or other sophisticated method to have this small run time gap. Without Gurobi the gap would be way bigger. But still there exists a gap we can't neglect for run time, the gap in efficiency exists but is relatively small.
...
\end{algorithm}
For this simulation we use $n=100$ and $m$ as the $x$-axis. Note that we run 25 times the random first-fit method to get the plot "mean random first-fit"
\begin{figure}[!h]
    \centering
...
\section{Scenario reduction and two-stage stochastic programming for another metric}
