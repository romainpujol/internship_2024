...
We've seen that backward algorithm isn't interesting in terms of CPU time. We'll compare now the run time of the Forward algorithm and the MILP formulation for $n\in\left[100,1000\right]$ because \Cref{time for bac} isn't relevant as Gurobi is very efficient with small size. Let's say we want to reduce a $n$ atoms distribution to a $\frac{n}{4}$ atoms distribution, let's compare run time and efficiency for:
