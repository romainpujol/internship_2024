...
\subsection{Application to two-stage linear models}
...
\begin{equation}\label{lp stoch}
    \min\left\{c^Tx + \mathbb{E}_\xi\left(\inf_y q\left(\xi\right)^Ty \right)\: :\: Wy\left(\xi\right)=h\left(\xi\right)-T\left(\xi\right)x\:,\: y\left(\xi\right)\:,\: x\in X \right\}.
\end{equation}
...
\end{corollary}
\begin{proof}
    This is an application of \Cref{stability} with $p=1$ where both assumptions are sufficient to ensure that conditions (2), (3) of \Cref{stability} are verified. See \cite[Proposition 22]{romisch_stability_2003}.
\end{proof}
\begin{remark}
