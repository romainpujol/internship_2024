...
\begin{remark}
    ...
    $$
    \min_{y\in\RR^d}f(y)=\min_{y\in\RR^d}\sum_{i\in I}\lVert x_i-y\rVert^2.
    $$
    We have to minimize an unconstrained function, when the norm is the $L-2$ norm, the gradient of this function is : 
    $$
    \nabla f(y)=-2\sum_{i\in I}\left(x_i-y\right).
    $$
    This gradient is worth zero when :
    $$
    y=\frac{1}{n}\sum_{i\in I}x_i=\text{mean}\left(I\right).
    $$
\end{remark}
...
\end{theorem}
The use of $\tau$ in the proof is legit as $\tau \leq \sum_{j\in J}\sum_{i\in I_j}\lVert x_i-\text{mean}(I_j)\rVert_2^2,\:\forall \{I_j\}$ means that $\tau\leq\min_{\{I_j\}}\sum_{j\in J}\sum_{i\in I_j}\lVert x_i-\text{mean}(I_j)\rVert_2^2$ but as we want to maximize the function $\tau$ will be the highest he can, so it'll be equal to the minimum, thereafter it gets technical but understandable.
