...
# 1) possibilité d'utiliser set au lieu de list pour remove plus efficacement (O(1)
# vs O(n))
# 2) éviter qq redondances d'affectations
# 3) rendre juste les indices pour caractériser les atomes 
# 4)? rendre juste la valeur de la distance plutôt que minimum. Cela part de
# l'hypothèse qu'on a pas besoin de rendre la distrib Q mais juste la valeur du
# pb. Si on veut le transport plan, peut être faire une autre fonction.
# L'idée c'est en faire une "minimaliste" dans le cas où on veut juste la valeur
# de la distance. On ajoute qd même les indices parce que ça coûte pas cher,
# pour warm start les méthodes intelligentes 
